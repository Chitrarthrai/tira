...
const API_URL = "http://localhost:5000/models"; // Your backend API endpoint
  ...
  return (
    <TransformControls
      ...
      onMouseUp={() => {
        if (orbitControlsRef.current) orbitControlsRef.current.enabled = true;
        enforceGroundLimit(model);
        onTransformChange(id); // Send updated data to the backend
      }}
      camera={camera}
    >
      <primitive object={model} castShadow />
...
const ShelfWithDragDrop: React.FC<Props> = ({
  ...
  const handleTransformChange = useCallback((id: number) => {
    setDroppedModels((prevModels) =>
      prevModels.map((model) => {
        if (model.id === id) {
          const updatedModel: DroppedModel = {
            ...model,
            position: [
              model.model.position.x,
              model.model.position.y,
          ...
          // Update specific user's model data
          fetch(`${API_URL}/${userId}/${id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              position: updatedModel.position,
              rotation: updatedModel.rotation,
            }),
  ...
  const loadDefaultShelf = useCallback(() => {
    const dracoLoader = new DRACOLoader();
    ...
    const fetchUserModels = async () => {
      try {
        ...
        if (userModels && userModels.length > 0) {
          ...
          userModels.forEach((savedModel: SavedModelState) => {
            gltfLoader.load(
              savedModel.url,
              (gltf) => {
                const model = gltf.scene;
                ...
                setDroppedModels((prev) => [
                  ...prev,
                  {
                    id: savedModel.id || modelIdCounter.current++,
                    model: model,
                    position: savedModel.position,
                    rotation: savedModel.rotation,
                    url: savedModel.url,
          ...
          // Load default shelf if no user models exist
          loadDefaultShelf();
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!isInitialLoad && droppedModels.length > 0) {
      // Save or update user's models
      ...
      }).catch(error => console.error("Error saving models:", error));
    }
    setIsInitialLoad(false);
  }, [droppedModels, userId, isInitialLoad]);
